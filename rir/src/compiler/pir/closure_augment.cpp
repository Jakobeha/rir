...
ClosureAugments ClosureAugments::
operator|(const ClosureAugments& o) const {
    return ClosureAugments((EnumSet<ClosureAugment>)*this | (EnumSet<ClosureAugment>)o, signature | o.signature);
}
...
std::ostream& operator<<(std::ostream& out, const ClosureAugments& augs) {
    ...
    if (!augs.signature.isAny) {
        if (!augs.empty() || augs.argumentForceOrder.size() > 0)
            out << ", ";
